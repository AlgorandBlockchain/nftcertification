<?
 ...
 switch($_GET['cari']){
	case "rekening" :
		if ($_GET['mode']=="induk") {
 			$query = mysql_query("SELECT accountname FROM chartmaster WHERE accountcode = '$id'");
		}
		if ($_GET['mode']=="rekp") {
 			$query = mysql_query("SELECT accountname FROM chartmaster WHERE accountcode = '$id'");
		}
	...
	case "supplier" :
		...
		if($_GET['mode']=="namarek"){
			$query = mysql_query("SELECT accountname FROM chartmaster a, supplier b WHERE a.accountcode = b.norek AND b.kode = '$id'");
		}
	break;
	case "pembeli" :
		if($_GET['mode']=="nama"){
		...
		if($_GET['mode']=="namarek"){
			$query = mysql_query("SELECT accountname FROM chartmaster a, konsumen b WHERE a.accountcode = b.norek AND b.kode = '$id'");
		}
	break;
	case "barang" :
		if($_GET['mode']=="nama"){
  ...
  case "karyawan" :
		if($_GET['mode']=="kode"){
			$query = mysql_query("SELECT kode FROM tmkaryawan WHERE kode = '$id'");
		}
	break;
  case "penawaran" :
		if($_GET['mode']=="kode"){
			$query = mysql_query("SELECT no_penawaran FROM tpenawaran WHERE no_penawaran = '$id'");
		}
	break;
 }
