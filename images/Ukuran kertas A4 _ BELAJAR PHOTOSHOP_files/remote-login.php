...
function showHideIDC(page)
{
	...
	else
	{
		...
		IDReplaceHtml($id('idc-showcomments-link'), "Tampilkan Komentar");
		$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
		$id('idc-showcomments-link').style.display = showCommentsStyle;
		IDReplaceHtml($id('idc-postcomment-link'), "Tampilkan Komentar");
...
function showIDC(page)
{
	...
	showCommentsStyle = $id('idc-showcomments-link').style.display;
	postCommentStyle = $id('idc-postcomment-link').style.display;
	IDReplaceHtml($id('idc-showcomments-link'), "Sembunyikan Komentar");
	$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
	$id('idc-showcomments-link').style.display = showCommentsStyle;
	IDReplaceHtml($id('idc-postcomment-link'), "Sembunyikan Komentar");
	$id('idc-postcomment-link').href = 'javascript:showHideIDC();';
	$id('idc-postcomment-link').style.display = postCommentStyle;
	if($id('idc-footer'))
...
function voteComment(commentid, vote) {
	...
	{
		showMsgBox("Maaf", "<p>Kamu sudah memberi suara atas komentar itu</p>", 0, listObj);
		return;
	}
	
	if(commentObj.comments[commentid] && (commentObj.comments[commentid].status==2 || commentObj.comments[commentid].status==6))
	{
		showMsgBox("Maaf", "<p>Kamu tak bisa memberi suara untuk komentar yang sudah dihapus</p>", 0, listObj);
		return;
	}
	...
	{
		showMsgBox("Maaf", "<p>Kamu sudah memberi suara atas komentar itu</p>", 0, $id("IDComment"+commentid));
		return;
	}
...
{	
	IDReplaceHtml($id("adminOptions"), 'Menyiapkan...');
	$id("adminOptions").href = "javascript:void(0);";		
	
	if(!commentObj.curUser.isLoggedIn)
	{
		showMsgBox("Maaf", "<p>Kamu harus masuk log untuk mengubah pengaturan.</p>", 0, listObj);
		return;
	}
	//changeDisabledLink();	
...
function postComment(src)
{
	...
	if(src==0) //new thread	
	{
		if( commentObj.postUsingTwitter ) {
			...
			{
				showMsgBox("Maaf", "<p>Komentar kamu terlalu panjang. Coba bagi menjadi beberapa komentar.");
				return;
			}
				
			IDReplaceHtml($id("IDNewThreadSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Mengirimkan..." />');
		...
		{
			if($id('IDCommentNewThreadText').value=='Masukkan teks disini!' || $id('IDCommentNewThreadText').value.length==0)
			{
				showMsgBox("Maaf", "<p>Tampaknya kamu belum menulis apapun.</p>", 0);
				return;
			}
			...
			if(commentObj.newthreadType == 4) //signup
			{
				...
				{
					showMsgBox("Maaf", "<p>Kamu belum menuliskan URI OpenID</p>", 0);
					return;
				}
			...
			else //login
			{
				if($id('txtOpenIDSignupNewThreadURL').value.length == 0 || $id('txtOpenIDSignupNewThreadURL').value=="http://")				
				{
					showMsgBox("Maaf", "<p>Kamu harus menuliskan sebuah uri openid.</p>", 0);
					return;
				}
		...
		//Check for defaults
		if($id('IDCommentNewThreadText').value=='Masukkan teks disini!')
		{
			showMsgBox("Maaf", "<p>Tampaknya kamu belum menulis apapun.</p>", 0);
			return;
		}		
		...
		{
			showMsgBox("Maaf", "<p>Untuk membuat sebuah akun, kamu membutuhkan sebuah alamat email yang valid</p>", 0);
			return;
		}
		
		if($id('txtNameNewThread').value == '' && commentObj.newthreadType == 0)
		{
			showMsgBox("Maaf", "<p>Tulis dulu nama kamu sebelum mengirim komentar</p>", 0);
			return;
		}

		/*if($id('txtNameNewThread').value.indexOf('&')>0)
		{
			showMsgBox("Maaf", "<p>Nama kamu tak boleh mengandung karakter &</p>", 0);
			return;
		}*/			
	...
	else
	{
		if( commentObj.postUsingTwitter ) {
			...
			IDReplaceHtml($id("IDReplyDivSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Mengirimkan..." />');		
			var requestObj = new buildRequestObj(theStr, 0, null, connectionErr);
			xs.make_request(requestObj);
		...
		{
			if($id('txtComment').value=='Masukkan teks disini!' || $id('txtComment').value.length==0)
			{
				showMsgBox("Maaf", "<p>Tampaknya kamu belum menulis apapun.</p>", 0);
				return;
			}
		...
		//Check for defaults
		if($id('txtComment').value=='Masukkan teks disini!')
		{
			showMsgBox("Maaf", "<p>Tampaknya kamu belum menulis apapun.</p>", 0);
			return;
		}		
...
{
	IDReplaceHtml($id("IDReplyDivSubmitLI"), '<a class="idc-btn_l-secondary" href="javascript: hideReply();"><span></span><span class="idc-r">Batal</span></a><a class="idc-btn_l" href="javascript: postComment(1);"><span></span><span class="idc-r"><strong>Kirim Komentar</strong></span></a>');
	IDReplaceHtml($id("IDNewThreadSubmitLI"), '<a class="idc-btn_l" href="javascript: postComment(0);"><span></span><span class="idc-r"><strong>Kirim Komentar</strong></span></a>');
	showMsgBox("Maaf","<p>Koneksi bermasalah, terjadi time out.</p>", 2);	
};
...
function showReply(commentid) {
	...
	if( commentObj.parentid )
		IDReplaceHtml($id("IDCommentPostReplyLink" + commentObj.parentid), "<a class='idc-btn_s' href='javascript: showReply(" + commentObj.parentid + ")'><span><input type='hidden' id='reqUsersOn' value='no' /></span><span class='idc-r'>Jawab</span></a>");
		
	IDReplaceHtml($id("IDCommentPostReplyLink"+commentid), "<span class='idc-btn_s'><span></span><span class='idc-r'>Mengirim...</span></span>");

		
	createCookie("IDReplyCommentId", commentid, 1);
	
	if($id("IDCommentReplyName") && commentObj.comments[commentid].displayName)
		IDReplaceHtml($id("IDCommentReplyName"), "Menjawab #user# ".replace(/#user#/, commentObj.comments[commentid].displayName));
...
{	
	...
	p.className="idc-bottom";
		...
		IDReplaceHtml(link2, "<span></span><span class='idc-r'>Menutup Pesan</span>");
...
{	
	...
	var h6 = $newEl("h6");
	var type = 0;
	var header = 'Laporkan komentar ini karena melakukan pelanggaran';
	IDReplaceHtml(h6, header);
	var pText = $newEl('div');
	var text = '<p>'+$id('IDCustomReportTxt').innerHTML+'</p><p>Alasan pelaporan:</p><textarea id="IDCCommentAdditional" class="idc-text_noresize" value="" style="width:90%;"></textarea><p><a href="javascript: reportThisComment('+commentId+');" class="idc-btn_s"><span></span><span class="idc-r">Laporkan ini!</span></a></p>';
...
function showReputationWhy() {
	showMsgBox("Meteran Reputasi IntenseDebate", "<p>Meteran Reputasi menggambarkan kekuatan dari seluruh komentar yang dibuat di sistem kami dimana setiap komentar dapat dihakimi oleh setiap pembaca lainnya. Ini adalah sebuah cara untuk mengetahui apakah sebuah komentar yang anda baca dibuat oleh seseorang yang dihormati.</p> <img src='"+IDHost+"/images/id-rep_scale.png' alt='Skala Reputasi' />", 0 );
};

function showForgotPassword()
{	
	showMsgBox("Forgot your password?", "<form><label for=\"txtResolveEmail\">Tulis alamat email kamu:</label><input type=\"text\" class=\"idc-text\" id=\"txtResolveEmail\" style=\"color:black\" onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><a href=\"javascript: forgotPassword();\" class=\"idc-btn_s\"><span></span><span class='idc-r'>Ingatkan aku kata sandi ku!</span></a></form>", 0);
...
{	
	showMsgBox("Punya umpan balik? Bagus!", "<form><label for='txtFeedbackEmail'>Email:</label><input type='text' id='txtFeedbackEmail' name='txtEmail' class='idc-text' onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><textarea name='txtFeedback' class='idc-text' id='txtFeedback' value='Feedback' onfocus=\"txtOnFocus(this, 'Feedback');\" onblur=\"txtOnBlur(this, 'Feedback');\"></textarea><a href='javascript: sendFeedback()' class='idc-btn_s'><span></span><span class='idc-r'>Kirim umpan balik</span></a></form>", 0);
...
{
	showMsgBox("Untuk apa mendaftar?", "<p><ul><li>Berteman dengan orang-orang yang mengomentari blog-blog favorit kamu</li><li>Biarkan dirimu dikenal dengan menyediakan tautan ke Facebook, Twitter, dan akun milikmu yang lain.</li><li>Perbanyak jumlah pembaca blog atau website kamu</li><li>Ijinkan teman-teman untuk melacak semua komentarmu di semua blog yang menggunakan komentar ID.</li></ul></p>", 0);
};
...
function showSignupNewThread()
{
    ...
    //$id("IDCommentsNewThreadListItem2").innerHTML='<h6><a href="javascript: showLoginNewThread();">Masuk log selain</a></h6>';
...
function showSignupReply()
{
	...
	//$id("IDCommentReplyLoginLink").innerHTML = '<h6><a href="javascript: showLoginReply();">Masuk log</a></h6>';	
...
function showSignupOpenIDReply()
{
	...
	//$id("IDCommentReplyLoginLink").innerHTML = '<h6><a href="javascript: showSignupReply();">Kembali</a></h6>';
	//$id("IDPostReplyUseOpenID").style.display="none";
...
function collapseThread(commentid) {
	if($id("IDCommentCollapseLink" + commentid) && $id("IDCommentCollapseLink" + commentid).className == "idc-collapselink") {
		...
		if( $id("IDCommentSubThread" + commentid).childNodes.length == 0 ) { //content not loaded yet
			...
			IDReplaceHtml($id("IDCommentSubThread" + commentid), "<div class='idc-thread_loading'><img src='http://s.intensedebate.com/images/ajax-loader.gif' alt='Menyiapkan...' /> Menyiapkan...</div>");
...
function IDeditComment(commentid)
{
	...
	{
		showMsgBox("Maaf", "<p>Kamu harus masuk log dulu untuk mengedit komentar.</p>", 0, null);		
		return;
	}
	
	if(!commentObj.comments[commentid] || !commentObj.comments[commentid].commentid)
	{
		showMsgBox("Maaf", "<p>Komentar itu tak ditemukan</p>", 0, null);		
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<a href="javascript: IDcancelEditComment('+commentid+');" class="idc-btn_l-secondary"><span></span><span class="idc-r">Batal</span></a><a href="javascript: IDsaveComment('+commentid+');" class="idc-btn_l"><span></span><span class="idc-r"><strong>Simpan</strong></span></a>');
...
function IDsaveComment(commentid)
{
	if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
	{
		showMsgBox("Maaf", "<p>Kamu harus masuk log untuk menghapus komentar</p>", 0, null);		
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Mengirimkan..." />');
...
{
	var answer = confirm("Kamu yakin ingin menghapus komentar ini?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
		...
		if(type<0 || type>1)
		{
			showMsgBox("Maaf", "<p>Tipe itu tidak valid</p>", 0, null);		
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkDelete"+commentid), 'Menghapus...');
...
{
	var answer = confirm("Kamu yakin ingin menghapus jejakbalik ini?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Maaf", "<p>Kamu harus masuk log untuk menghapus jejakbalik.</p>", 0, null);		
			return;
		}
		...
		{
			IDReplaceHtml($id("IDTrackBackDeleteLink"+trackbackid), 'Menghapus...');
		...
		{
			showMsgBox("Maaf", "<p>Jejakbalik itu tidak ditemukan...</p>", 0, null);
			return;		
		}
	}	
}
...
{
	var answer = confirm("Kamu yakin ingin memblokir alamat IP ini?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Maaf", "<p>kamu harus masuk log untuk memblokir sebuah IP.</p>", 0, null);		
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkBan"+commentid), 'Memblokir...');
...
function IDCSubscribeByEmail(type)
{
	...
	IDReplaceHtml($id('IDCSubscribeSubmit'+type), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Mengirimkan..." />');
...
IDC.user_menu = {
    ...
    "add_friend": function ( uid ) {
            if ( commentObj && commentObj.curUser && !commentObj.curUser.isLoggedIn ) {
                    showMsgBox("Maaf", "<p>Kamu harus masuk log dulu untuk menambahkan teman</p>", 0, null);
                    return;
            }
...
IDC.user_menu.clear_cache();
	...
	var IDCounted;
	_qoptions = [{ labels:"languages.id_ID,idc", qacct:"p-94D6e1NDscLvI"},{qacct:"p-18-mFEk4J448M", labels:"type.intensedebate.embed"}];
	IDcountIT();
}
